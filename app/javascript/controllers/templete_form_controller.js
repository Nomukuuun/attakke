...
// Connects to data-controller="templete-form"
export default class extends Controller {
  ...
  connect() {
    console.log("fieldRowTargets:", this.fieldRowTargets);
    this.setInitialIconState()
  }

  // チェックアイコンをupdateの度にquantityの値を基に表示する
  setInitialIconState() {
    this.fieldRowTargets.forEach((row) => {
      const existQuantity = row.querySelector("[data-templete-form-target='existQuantity']");
      const checkedIcon = row.querySelector("[data-templete-form-target='checkedIcon']");
      const notCheckedIcon = row.querySelector("[data-templete-form-target='notCheckedIcon']");
  ...
  toggleIcon(event) {
    const fieldRow = event.target.closest(
      "[data-templete-form-target='fieldRow']"
    );
    const checkedIcon = fieldRow.querySelector(
      "[data-templete-form-target='checkedIcon']"
    );
    const notCheckedIcon = fieldRow.querySelector(
      "[data-templete-form-target='notCheckedIcon']"
    );
    const existQuantity = fieldRow.querySelector(
      "[data-templete-form-target='existQuantity']"
    );
  ...
  changeField(event) {
    ...
    const existField = fieldRow.querySelector(
      "[data-templete-form-target='existField']"
    );
    const numField = fieldRow.querySelector(
      "[data-templete-form-target='numField']"
    );
    const model = fieldRow.querySelector(
      "[data-templete-form-target='model']"
    );
  ...
  incrementNumber(event) {
    ...
    const numQuantity = fieldRow.querySelector(
      "[data-templete-form-target='numQuantity']"
    );
